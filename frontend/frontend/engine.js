...
const elements = {
  ...
  animatePlasma: document.getElementById("animatePlasma"),
  backendResolution: document.getElementById("backendResolution"),
  backendSteps: document.getElementById("backendSteps"),
  backendRender: document.getElementById("backendRender"),
  backendCancel: document.getElementById("backendCancel"),
  backendStatus: document.getElementById("backendStatus"),
  backendImage: document.getElementById("backendImage"),
  backendLoading: document.getElementById("backendLoading"),
  gpuViewButton: document.getElementById("gpuViewButton"),
  pythonViewButton: document.getElementById("pythonViewButton"),
...
function renderLoop(now) {
  ...
  const animate = elements.animatePlasma.checked;
  const minimumInterval = dragging ? 34 : animate ? 50 : 0;

  if (!needsRender && !animate) return;
  if (now - lastDrawTime < minimumInterval) return;
...
function showGpuView() {
  canvas.style.display = "block";
  elements.backendImage.style.display = "none";
  elements.gpuViewButton.classList.add("active");
  elements.pythonViewButton.classList.remove("active");
  elements.viewModeLabel.textContent = "REALTIME GPU";
  invalidate();
}

function showPythonView() {
  if (!elements.backendImage.src) return;
  canvas.style.display = "none";
  elements.backendImage.style.display = "block";
  elements.gpuViewButton.classList.remove("active");
  elements.pythonViewButton.classList.add("active");
  elements.viewModeLabel.textContent = "PYTHON REFERENCE";
}

elements.gpuViewButton.addEventListener("click", showGpuView);
elements.pythonViewButton.addEventListener("click", showPythonView);
elements.animatePlasma.addEventListener("change", () => {
  stableAnimationTime = performance.now() * 0.001;
  invalidate();
});
...
function normaliseImageUrl(rawPath) {
  ...
  if (!raw) return "/static/render.png";
  if (raw.includes("backend/static/")) {
    return "/static/" + raw.split("backend/static/")[1];
  }
...
async function runBackendRender() {
  if (backendController) return;

  backendController = new AbortController();
  elements.backendRender.disabled = true;
  elements.backendCancel.disabled = false;
  elements.backendLoading.classList.add("show");
  elements.backendStatus.textContent = "Python renderer running. Do not start another request.";

  const payload = {
    mass: state.mass,
    spin: state.spin,
    observer_radius: state.distance,
    fov: state.fov,
    resolution: Number(elements.backendResolution.value),
    steps: Number(elements.backendSteps.value)
  };
  ...
  try {
    ...
    const imageUrl = normaliseImageUrl(result.image) + "?v=" + Date.now();

    elements.backendImage.src = imageUrl;
    await elements.backendImage.decode().catch(() => {});
    elements.pythonViewButton.disabled = false;
    showPythonView();
    ...
    elements.backendRender.disabled = false;
    elements.backendCancel.disabled = true;
    elements.backendLoading.classList.remove("show");
  }
}

elements.backendRender.addEventListener("click", runBackendRender);
elements.backendCancel.addEventListener("click", () => {
  if (backendController) backendController.abort();
});
